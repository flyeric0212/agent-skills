...
- **跨会话保留上下文**：会话会丢失工作记忆，被暂停的工作重新上手要花时间重新摸清。`handoff` 写入检查点——可验证的进展、决策和可直接执行的下一步——让下一会话或 Agent 精确地从工作停下的地方继续。
...
| [handoff](./skills/handoff/SKILL.md) | 生成工程交接检查点（`handoff.md`），记录可验证的进展、决策和下一步，让下一会话或 Agent 继续未完成的工作。 |
...
1. **写检查点**——严格按照内置模板生成（或更新已有）`handoff.md`；存在用户指定路径和项目约定时优先遵守。
2. **只写有证据的进展**——只有可验证的结果才计入 Done，推测明确标记；更新时以现有 handoff 作为上一个检查点。
3. **可直接继续**——每个检查点都以下一 Agent 能直接执行的第一项 Next Step 结尾，暂停的工作无需重新摸清上下文即可继续。
